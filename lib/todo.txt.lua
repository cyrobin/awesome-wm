-- Todo.txt display through specific Quake-like console
-- Allow for another quake-like console, for todo.txt management purpose
-- Similar to:
...
-- are able to detect the ToDo console from its name
-- (ToDoConsoleNeedsUniqueName by default).
...
-- ToDoConsoleNeedsUniqueName. Otherwise, you may run into problems
-- with focus.
...
-- Create a console
function ToDoConsole:new(config)
   ...
   --config.name     = config.name     or "ToDoConsoleNeedsUniqueName" -- window name
   config.name     = config.name     or "Todo.txt-Console" -- window name
   config.argname  = config.argname  or "-name %s"     -- how to specify window name

   -- If width or height <= 1 this is a proportion of the workspace
   config.height   = config.height   or 0.90	       -- height
   config.width    = config.width    or 0.25     -- width
   config.vert     = config.vert     or "center"	       -- top, bottom or center
   config.horiz    = config.horiz    or "left"       -- left, right or center

   config.screen   = config.screen or capi.mouse.screen
   config.visible  = config.visible or false -- Initially, not visible

   local console = setmetatable(config, { __index = ToDoConsole })
   ...
   -- "Reattach" currently running ToDoConsole. This is in case awesome is restarted.
...
-- Toggle the console
function ToDoConsole:toggle()
   self.visible = not self.visible
   self:display()
end

setmetatable(_M, { __call = function(_, ...) return ToDoConsole:new(...) end })
