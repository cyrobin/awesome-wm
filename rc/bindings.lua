...
config.keys.global = awful.util.table.join(
   ...
   awful.key({ modkey,           }, "Tab", 
        ...
        if client.focus then client.focus:raise() end end,                                              "Focus previously focused window"),
...
config.keys.client = awful.util.table.join(
   ...
   awful.key({ modkey,           }, "x",      function (c) c:kill()                         end,        "Close"),
...
config.keys.global = awful.util.table.join(
   ...
   awful.key({ modkey, "Control" }, "t",    function () screen_focus( 1) end,                           "Jump to next screen"),
   awful.key({ modkey, "Control" }, "s",    function () screen_focus(-1) end),

   keydoc.group("Layout manipulation"),
   awful.key({ modkey,           }, "r",     function () awful.tag.incmwfact( 0.05)    end,             "Increase master-width factor"),
   awful.key({ modkey,           }, "c",     function () awful.tag.incmwfact(-0.05)    end,             "Decrease master-width factor"),
   awful.key({ modkey, "Shift"   }, "r",     function () awful.tag.incnmaster( 1)      end,             "Increase number of masters"),
   awful.key({ modkey, "Shift"   }, "c",     function () awful.tag.incnmaster(-1)      end,             "Decrease number of masters"),
   awful.key({ modkey, "Control" }, "r",     function () awful.tag.incncol( 1)         end,             "Increase number of columns"),
   awful.key({ modkey, "Control" }, "c",     function () awful.tag.incncol(-1)         end,             "Decrease number of columns"),
   awful.key({ modkey, "Shift"   }, "t",     function () awful.client.swap.byidx(  1)  end,             "Swap with next window"),
   awful.key({ modkey, "Shift"   }, "s",     function () awful.client.swap.byidx( -1)  end,             "Swap with previous window"),

   keydoc.group("Misc"),
   awful.key({ modkey, "Control" }, "l", awesome.restart,                                               "Restart awesome")
)

config.keys.client = awful.util.table.join(
   config.keys.client,
   keydoc.group("Window-specific bindings"),
   awful.key({ modkey,           }, "j",      function (c) c:raise()            end,                    "Raise window"),
   awful.key({ modkey,           }, "k",      function (c) c.sticky = not c.sticky end,                 "Stick window")
)
--}}}
